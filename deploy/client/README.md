...
```powershell
powershell -ExecutionPolicy Bypass -File .\deploy\client\generate-update-manifest.ps1 -Version 0.1.4 -PackagePath artifacts\client-installer\win-x64\YourVpnClient-0.1.4.msi -PackageBaseUrl https://downloads.example.com/vpn-client -OutputPath artifacts\client-installer\win-x64\update-manifest.json
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\deploy\client\publish-update-origin.ps1 -Version 0.1.4 -ServerPassword <root-password> -ReleaseNotes "0.1.4 desktop shortcut fix and minimal update action." -UploadZip
```
