...
- но для пары старых нод это уже не выглядит как баг генерации конфигов.

### 4. Main open gap

На `5.61.37.29` основная operational часть уже в порядке:

- node agent зарегистрирован в control plane;
- нода перешла в `Healthy`;
- sidebar закрепляет её первой.

Но это не закрывает текущий главный риск:

- совместимость нашей выдачи с third-party import path Amnezia всё ещё не доказана до конца;
- server-side provisioning уже близок к upstream;
- остаточная проблема выглядит как client import/runtime compatibility issue, а не как базовая ошибка создания peer.

### 5. Что подтвердилось по проблемным нодам

На `2026-03-18` мы заменили две проблемные ноды Inferno:

- `38.180.137.249` -> `5.61.37.83`
- `45.136.49.191` -> `5.61.41.204`

Практический вывод после замены:

- проблема с "handshake есть, а интернета нет" для этих двух узлов действительно была связана с локацией / сетевым путём;
- это подтвердилось тем, что тестовые доступы, выданные через нашу control plane на обе новые ноды, начали работать нормально;
- значит сама генерация peer-конфига не была корнем именно для этих двух серверов.
